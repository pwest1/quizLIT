...
class QuizApp:
    def __init__(self, root):
        ...
        self.upload_quiz_button = tk.Button(root, text="Upload", command=self.upload_quiz,
                                            font=("Futura", 14, 'bold'), height="2", width="10", bg='white', fg='black',
                                            highlightthickness=0, border=0)
        self.upload_quiz_button.pack(pady=10)
    ...
    def load_from_excel(self, file_path):
        ...
        for row in worksheet.iter_rows(min_row=2, values_only=True):
            question_text, correct_answer = row
            self.add_question(Question(question_text, correct_answer))
            self.wrong_answers.append(correct_answer)
    ...
    def upload_quiz(self):
        ...
        if file_path:
            try:
                ...
                messagebox.showinfo("Quiz Loaded", "Quiz questions loaded successfully.")
            except Exception as e:
                messagebox.showerror("Error", f"An error occurred while loading the quiz: {str(e)}")

    def add_question(self, question):
        self.question_list.add_question(question)
